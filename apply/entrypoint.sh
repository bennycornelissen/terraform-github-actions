...
set +e
OUTPUT=$(sh -c "TF_IN_AUTOMATION=true terraform apply -no-color -input=false -auto-approve -var=\"env_name_ci=ci-$GITHUB_SHA\" $*" 2>&1)
SUCCESS=$?
echo "$OUTPUT"
set -e
...
$DESTROY_OUTPUT"
    ...
    COMMENT="#### ${GITHUB_WORKFLOW} - ${GITHUB_ACTION}: \`terraform apply\` Success
$OUTPUT"
fi
